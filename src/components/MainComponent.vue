    ...
    <ul class="wrapper">
      <li class="card" v-for="(movie) in moviesComputed" :key="movie.id" @mouseenter="getActorsMovie(movie.id)">
        ...
        <div class="card-img-2">
          ...
          <span>Generi:</span>
          <span v-for="(genre,index) in movie.genre_ids" :key="index + 800">
            <span v-for="(genreG, index2) in genresId" :key="index2">
              <span v-if="genreG === genre">
                {{genresName[index2]}},
              </span>
            </span>
...
export default {
  ...
  methods: {
    ...
    getGenres() {
      axios
        .get(`https://api.themoviedb.org/3/genre/movie/list?api_key=8cbfe3a39283c4f44f68376b0e0e6c1b&language=en-US`)
        .then((res) => {
          this.genres.push(res.data)

          for (let i = 0; i < this.genres[0].genres.length; i++) {
            const el = this.genres[0].genres[i];
            this.genresId.push(el.id)
            this.genresName.push(el.name)
            
          }
          console.log(this.genresId, this.genresName)
        })
    }
